...
#endif
    pde_t *pdep = &pgdir[PDX(la)]; // find page directory entry »ñÈ¡´«ÈëµÄÐéÄâµØÖ·ÖÐËù¶ÔÓ¦µÄÒ³Ä¿Â¼ÏîµÄÎïÀíµØÖ·
...
c0103315:	89 45 f4             	mov    %eax,-0xc(%ebp)
    if (!(*pdep & PTE_P)) { // check if entry is not present ÅÐ¶Ï¸ÃÒ³Ä¿Â¼ÏîÊÇ·ñÓÐÐ§£¬Èç¹ûÎÞÐ§ÔòÐèÒª½øÐÐ·ÖÅä
...
c0103342:	75 0a                	jne    c010334e <get_pte+0x55>
            return NULL; // Èç¹û·ÖÅäÒ³ÃæÊ§°Ü£¬»òÕß²»ÔÊÐí·ÖÅä£¬Ôò·µ»ØNULL
c0103344:	b8 00 00 00 00       	mov    $0x0,%eax
c0103349:	e9 ca 00 00 00       	jmp    c0103418 <get_pte+0x11f>
        }
        set_page_ref(page, 1); // ÉèÖÃ¸ÃÎïÀíÒ³ÃæµÄÒýÓÃ´ÎÊýÎª1
...
c010335b:	83 c4 10             	add    $0x10,%esp
        uintptr_t pa = page2pa(page); // get linear address of page »ñÈ¡µ±Ç°ÎïÀíÒ³ÃæËù¹ÜÀíµÄÎïÀíµØÖ·
...
c010336c:	89 45 ec             	mov    %eax,-0x14(%ebp)
        memset(KADDR(pa), 0, PGSIZE); // clear page content using memset Çå¿Õ¸ÃÎïÀíÒ³ÃæµÄÊý¾Ý¡£ÐèÒª×¢ÒâµÄÊÇÊ¹ÓÃÐéÄâµØÖ·
...
c0103390:	68 73 01 00 00       	push   $0x173
...
c01033c5:	89 10                	mov    %edx,(%eax)
        // set page directory entry's permission ½«ÐÂ·ÖÅäµÄÒ³ÃæÉèÖÃÎªµ±Ç°È±Ê§µÄÒ³Ä¿Â¼ÌõÄ¿ÖÐ,Ö®ºó¸ÃÒ³Ãæ¾ÍÊÇÆäÖÐµÄÒ»¸ö¶þ¼¶Ò³Ãæ
    }
    return &((pte_t *)KADDR(PDE_ADDR(*pdep)))[PTX(la)]; // return page table entry ·µ»ØÔÚpgdirÖÐ¶ÔÓ¦ÓÚlaµÄ¶þ¼¶Ò³±íÏî
...
c01033ef:	68 77 01 00 00       	push   $0x177
...
c0103416:	01 d0                	add    %edx,%eax
    // ÓÉÓÚÕâÊ±ÒÑ¾­Æô¶¯ÁËÒ³»úÖÆ£¨ÔÚentry.SÖÐ£©£¬¶ÔÄÚ´æµÄ²Ù×÷Ê¹ÓÃµÄ¾ùÎªÄÚºËÐéµØÖ·¶ø·ÇÎïÀíµØÖ·£¬ËùÒÔ½øÐÐ²Ù×÷Ç°ÐèÒª½«ÎïÀíµØÖ·Í¨¹ýKADDR½øÐÐ×ª»»¡£
}
c0103418:	c9                   	leave  
c0103419:	c3                   	ret    
...
// ÊÍ·ÅÒ»¸öÓëÏßÐÔµØÖ· la Ïà¹ØÁªµÄ Page struct£¬²¢ÇÒÇåÀí£¨Ê¹ÎÞÐ§£©ÓëÏßÐÔµØÖ· laÏà¹ØµÄ pte¡£
...
#endif
    if (*ptep & PTE_P) // check if this page table entry is present£¬Èç¹û´«ÈëµÄÒ³±íÌõÄ¿ÊÇ¿ÉÓÃµÄ
...
c0103484:	74 50                	je     c01034d6 <page_remove_pte+0x62>
    {
        struct Page * page = pte2page(*ptep); // find corresponding page to pte£¬»ñÈ¡¸ÃÒ³±íÏîËù¶ÔÓ¦µÄµØÖ· 
...
c0103497:	89 45 f4             	mov    %eax,-0xc(%ebp)
        if (page_ref_dec(page) == 0) // decrease page reference£¬Èç¹û¸ÃÒ³µÄÒýÓÃ´ÎÊýÔÚ¼õ1ºóÎª0
...
c01034aa:	75 10                	jne    c01034bc <page_remove_pte+0x48>
            free_page(page); // and free this page when page reference reachs 0£¬ÊÍ·Åµ±Ç°Ò³
...
c01034b9:	83 c4 10             	add    $0x10,%esp
        *ptep = 0; // clear second page table entry£¬Çå¿ÕPTE
c01034bc:	8b 45 10             	mov    0x10(%ebp),%eax
c01034bf:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
        tlb_invalidate(pgdir, la); // flush tlb£¬Ë¢ÐÂTLBÄÚµÄÊý¾Ý
...
c01035fd:	68 db 01 00 00       	push   $0x1db
...
c010366f:	68 e8 01 00 00       	push   $0x1e8
...
c010369f:	68 e9 01 00 00       	push   $0x1e9
...
c01036d1:	68 ea 01 00 00       	push   $0x1ea
...
c0103713:	68 ee 01 00 00       	push   $0x1ee
...
c010374a:	68 f1 01 00 00       	push   $0x1f1
...
c0103779:	68 f2 01 00 00       	push   $0x1f2
...
c01037a5:	68 f3 01 00 00       	push   $0x1f3
...
c01037de:	68 f5 01 00 00       	push   $0x1f5
...
c0103822:	68 f6 01 00 00       	push   $0x1f6
...
c0103867:	68 f9 01 00 00       	push   $0x1f9
...
c01038a1:	68 fa 01 00 00       	push   $0x1fa
...
c01038c6:	68 fb 01 00 00       	push   $0x1fb
...
c01038eb:	68 fc 01 00 00       	push   $0x1fc
...
c0103912:	68 fd 01 00 00       	push   $0x1fd
...
c010393e:	68 fe 01 00 00       	push   $0x1fe
...
c0103973:	68 00 02 00 00       	push   $0x200
...
c010399f:	68 01 02 00 00       	push   $0x201
...
c01039ca:	68 02 02 00 00       	push   $0x202
...
c0103a04:	68 03 02 00 00       	push   $0x203
...
c0103a33:	68 04 02 00 00       	push   $0x204
...
c0103a58:	68 05 02 00 00       	push   $0x205
...
c0103a97:	68 08 02 00 00       	push   $0x208
...
c0103ac2:	68 09 02 00 00       	push   $0x209
...
c0103b03:	68 0c 02 00 00       	push   $0x20c
...
c0103b2e:	68 0d 02 00 00       	push   $0x20d
...
c0103b6b:	68 0f 02 00 00       	push   $0x20f
...
c0103bf0:	68 1b 02 00 00       	push   $0x21b
...
c0103c30:	68 1b 02 00 00       	push   $0x21b
...
c0103c5c:	68 1c 02 00 00       	push   $0x21c
...
c0103cae:	68 1f 02 00 00       	push   $0x21f
...
c0103cd3:	68 1f 02 00 00       	push   $0x21f
...
c0103cf7:	68 21 02 00 00       	push   $0x221
...
c0103d3c:	68 25 02 00 00       	push   $0x225
...
c0103d68:	68 26 02 00 00       	push   $0x226
...
c0103d9d:	68 27 02 00 00       	push   $0x227
...
c0103dc9:	68 28 02 00 00       	push   $0x228
...
c0103e15:	68 2c 02 00 00       	push   $0x22c
...
c0103e58:	68 2f 02 00 00       	push   $0x22f
...
c010445f:	68 99 00 00 00       	push   $0x99
...
c01044db:	68 9c 00 00 00       	push   $0x9c
...
c01046e5:	68 b8 00 00 00       	push   $0xb8
...
c01047a6:	68 c9 00 00 00       	push   $0xc9
...
c01047d5:	68 ca 00 00 00       	push   $0xca
...
c0104804:	68 cb 00 00 00       	push   $0xcb
...
c0104835:	68 cd 00 00 00       	push   $0xcd
...
c0104884:	68 ce 00 00 00       	push   $0xce
...
c01048b8:	68 d0 00 00 00       	push   $0xd0
...
c01048ec:	68 d1 00 00 00       	push   $0xd1
...
c0104920:	68 d2 00 00 00       	push   $0xd2
...
c0104980:	68 d6 00 00 00       	push   $0xd6
...
c01049bc:	68 db 00 00 00       	push   $0xdb
...
c0104a0f:	68 e0 00 00 00       	push   $0xe0
...
c0104a3e:	68 e2 00 00 00       	push   $0xe2
...
c0104a6d:	68 e3 00 00 00       	push   $0xe3
...
c0104a9c:	68 e4 00 00 00       	push   $0xe4
...
c0104ac6:	68 e6 00 00 00       	push   $0xe6
...
c0104b09:	68 e9 00 00 00       	push   $0xe9
...
c0104b3a:	68 ec 00 00 00       	push   $0xec
...
c0104b64:	68 ed 00 00 00       	push   $0xed
...
c0104b86:	68 ef 00 00 00       	push   $0xef
...
c0104c44:	68 00 01 00 00       	push   $0x100
...
c0104c93:	68 03 01 00 00       	push   $0x103
...
c0104cc7:	68 08 01 00 00       	push   $0x108
...
c0104d0c:	68 09 01 00 00       	push   $0x109
...
c0104d6c:	68 0d 01 00 00       	push   $0x10d
...
c0104d96:	68 0e 01 00 00       	push   $0x10e
...
c0104de6:	68 14 01 00 00       	push   $0x114
...
c0104e3c:	68 15 01 00 00       	push   $0x115
...
c0104e6b:	68 16 01 00 00       	push   $0x116
...
c0104e95:	68 17 01 00 00       	push   $0x117
...
c0104eb9:	68 18 01 00 00       	push   $0x118
...
c0104f32:	68 1d 01 00 00       	push   $0x11d
...
c0104f82:	68 1e 01 00 00       	push   $0x11e
...
c0104fb6:	68 20 01 00 00       	push   $0x120
...
c0104ffa:	68 22 01 00 00       	push   $0x122
...
c0105049:	68 27 01 00 00       	push   $0x127
...
c0105073:	68 28 01 00 00       	push   $0x128
...
c0105095:	68 2a 01 00 00       	push   $0x12a
...
c010511b:	68 35 01 00 00       	push   $0x135
...
c010513a:	68 36 01 00 00       	push   $0x136
