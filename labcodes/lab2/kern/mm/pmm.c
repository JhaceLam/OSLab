...
get_pte(pde_t *pgdir, uintptr_t la, bool create) {
    ...
    if (!(*pdep & PTE_P)) { // check if entry is not present 判断该页目录项是否有效，如果无效则需要进行分配
        ...
        if (!create || (page = alloc_page()) == NULL) { // check if creating is needed, then alloc page for page table
            return NULL; // 如果分配页面失败，或者不允许分配，则返回NULL
        }
        ...
        *pdep = pa | PTE_U | PTE_W | PTE_P;
        // set page directory entry's permission 将新分配的页面设置为当前缺失的页目录条目中,之后该页面就是其中的一个二级页面
    }
    return &((pte_t *)KADDR(PDE_ADDR(*pdep)))[PTX(la)]; // return page table entry 返回在pgdir中对应于la的二级页表项
    // 由于这时已经启动了页机制（在entry.S中），对内存的操作使用的均为内核虚地址而非物理地址，所以进行操作前需要将物理地址通过KADDR进行转换。
}
...
// 释放一个与线性地址 la 相关联的 Page struct，并且清理（使无效）与线性地址 la相关的 pte。
